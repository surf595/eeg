...
def get_raw(file_id: int, amplitude_scale: float = 1.0) -> dict:
    ...
    n = data.signals.shape[1]
    max_points = 5000
    step = max(1, n // max_points)
    idx = list(range(0, n, step))
    times = [i / fs for i in idx]

    # downsample for browser transport only; backend analysis/export keeps full resolution
    spacing = max(1.0, float(data.signals.std()) * 5)
    stacked = []
    for i, ch in enumerate(data.channels):
        y = (data.signals[i][idx] * amplitude_scale) + i * spacing
...
def analyze(req: AnalyzeRequest) -> dict:
    ...
    db.add_feature_set(
        file_id=req.file_id,
        segment_start=req.start_sec,
        segment_end=req.end_sec,
        features_json=json.dumps(result["metrics"], ensure_ascii=False),
        created_at=now,
    )
